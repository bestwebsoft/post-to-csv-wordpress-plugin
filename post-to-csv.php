...
if ( ! function_exists ( 'psttcsv_plugin_init' ) ) {
	function psttcsv_plugin_init() {
		...
		/* Functionality for frontend "Export" button */
//        if ( isset( $_POST['psttcsv_export_submit_button'] ) ) {
//            psttcsv_export_button();
//        }
	...
	function psttcsv_print_csv() {
        ...
        if ( isset( $_POST['psttcsv_export_submit_comments'] ) ) {
            ...
            if ( in_array( 'permalink', $psttcsv_options['psttcsv_comment_fields'] ) ) {
                ...
                LIMIT " . $start * $limit . ", " . $limit . " "  , ARRAY_A );
            } else {
                $results = $wpdb->get_results( "
            ...
            $colArray = $psttcsv_options['psttcsv_comment_fields'];
            sort( $colArray );
            ...
            while ( ! empty( $results ) ) {
                foreach ( $results as $result ) {
                    ...
                    if ( $psttcsv_options['psttcsv_delete_html'] && isset( $result['comment_content'] ) ) {
                        $result['comment_content'] = strip_tags( $result['comment_content'] );
                    }
                    ksort( $result );
                    fputcsv( $file, $result, ';' );
                }
...
if ( ! function_exists( 'psttcsv_export_button_shortcode' ) ) {
    function psttcsv_export_button_shortcode() {
        if ( is_admin() ) {
            return false;
        }

        $page_info = get_queried_object();
        $terms = wp_get_post_terms( $page_info->ID, 'category' );

        if ( ! $terms ) {
            return '<p>This page does not have category.</p>';
        }
        ...
        $button_html .= '<button type="submit" name="psttcsv_export_submit_button" class="button" value="';
        $button_html .= $category_id;
        $button_html .= '" >Export category "';
        $button_html .= $category_name;
        $button_html .= '"</button></form>';

        return $button_html;
    }
}

/* Download posts with the same category as category of page where the button from shortcode placed on */
if ( ! function_exists( 'psttcsv_export_button' ) ) {
    function psttcsv_export_button () {
        ...
        $results = $wpdb->get_results( "
            SELECT `ID`, `post_type`, `post_content`, `post_author`, `object_id`, `term_taxonomy_id`
            FROM $wpdb->posts
        ...
        if ( ! empty( $results ) ) {
            $file = fopen( $filename, "w" );
            fputcsv( $file, array( 'Post author', 'Post content', 'Post type' ), ';' );
            while ( ! empty( $results ) ) {
                foreach ( $results as $result ) {
                    unset( $result['ID'] );
                    unset( $result['object_id'] );
                    unset( $result['term_taxonomy_id'] );
            ...
            fclose( $file );
            header( 'Content-Type: application/octet-stream' );
            header( 'Content-Disposition: attachment; filename="posts_category_'.$category_name.'.csv"' );
        }
        readfile( $filename );
        unlink( $filename );
        exit();
...
if ( ! function_exists( 'psttcsv_add_category_to_pages' ) ) {
    function psttcsv_add_category_to_pages() {
...
/* Functionality for frontend "Export" button */
//add_shortcode( 'psttcsv_export_button', 'psttcsv_export_button_shortcode' );
//add_action( 'init', 'psttcsv_add_category_to_pages' );