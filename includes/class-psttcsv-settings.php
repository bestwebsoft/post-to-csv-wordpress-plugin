	...
	class Psttcsv_Settings_Tabs extends Bws_Settings_Tabs {
		...
		public function save_options() {
			$message = $notice = $error = '';
			 $this->options['psttcsv_export_type'] = isset( $_POST['psttcsv_export_type'] ) && in_array( $_POST['psttcsv_export_type'], array( 'post_type', 'taxonomy' ) ) ? $_POST['psttcsv_export_type'] : 'post_type';

			if ( ! isset( $_POST['psttcsv_post_type'] ) ) {
				$error = __( 'Please choose at least one Post Type.', 'post-to-csv' ) . '<br />';
			} else {
				$this->options['psttcsv_post_type'] = array_map( 'sanitize_title', ( array )$_POST['psttcsv_post_type'] );
			}

			if ( ! isset( $_POST['psttcsv_fields'] ) ) {
				$error .= __( 'Please choose at least one Field.', 'post-to-csv' ) . '<br />';
			} else {
				$this->options['psttcsv_fields'] = array_map( 'sanitize_title', ( array )$_POST['psttcsv_fields'] );
				foreach ( $this->post_types as $post_type => $post_value ) {
					$this->options[ 'psttcsv_meta_key_' . $post_type ] = isset( $_POST[ 'psttcsv_meta_key_' . $post_type ] ) ? array_map( 'sanitize_title', ( array )$_POST[ 'psttcsv_meta_key_' . $post_type ] ) : array();
				}
			}
			foreach ( $this->post_taxonomies as $taxonomy ) {
                $this->options['psttcsv_taxonomy'][$taxonomy->name] = isset( $_POST[ 'psttcsv_term_taxonomy_' . $taxonomy->name ] ) ? array_map( 'sanitize_text_field', ( array )$_POST[ 'psttcsv_term_taxonomy_' . $taxonomy->name ] ) : array();
            }

			if ( ! isset( $_POST['psttcsv_status'] ) ) {
				$error .= __( 'Please choose at least one Post status.', 'post-to-csv' ) . '<br />';
			} else {
				$this->options['psttcsv_status'] = array_map( 'sanitize_title', ( array )$_POST['psttcsv_status'] );
			}

			$this->options['psttcsv_order'] = isset( $_POST['psttcsv_order'] ) && in_array( $_POST['psttcsv_order'], array( 'post_title', 'post_date', 'post_author' ) ) ? $_POST['psttcsv_order'] : 'post_date';
			$this->options['psttcsv_direction'] = isset( $_POST['psttcsv_direction'] ) && in_array( $_POST['psttcsv_direction'], array( 'asc', 'desc' ) ) ? $_POST['psttcsv_direction'] : 'desc';
			...
			$this->options['psttcsv_order_comment'] = isset( $_POST['psttcsv_order_comment'] ) && in_array( $_POST['psttcsv_order_comment'], array( 'comment_ID', 'comment_date', 'comment_author' ) ) ? $_POST['psttcsv_order_comment'] : 'comment_ID';
			$this->options['psttcsv_direction_comment'] = isset( $_POST['psttcsv_direction_comment'] ) && in_array( $_POST['psttcsv_direction_comment'], array( 'asc', 'desc' ) ) ? $_POST['psttcsv_direction_comment'] : 'desc';
		...
		public function tab_settings() { ?>
			...
			<table class="form-table psttcsv-table-settings" id="psttcsv_settings_form">
                <tr valign="top">
                    ...
                    <td><fieldset>
                            <label><input type="radio" name="psttcsv_export_type" value="post_type" <?php if ( 'post_type' == $this->options['psttcsv_export_type'] ) echo 'checked="checked"'; ?> /> <?php _e( 'Post Types', 'post-to-csv' ); ?></label><br />
                ...
                <tr valign="top" id="psttcsv-posttype-block">
					<th scope="row"><?php _e( 'Post Types', 'post-to-csv' ); ?></th>
					<td><fieldset>
							...
							<?php foreach ( $this->post_types as $post_type => $post_type_name ) { ?>
								<label>
									<input type="checkbox" name="psttcsv_post_type[]" value="<?php echo $post_type; ?>" class="bws_option_affect" data-affect-show="[data-post-type=<?php echo $post_type; ?>]" <?php checked( in_array( $post_type, $this->options['psttcsv_post_type'] ) ); ?> /> 
				...
				<tr>
					...
					<td>
						<input type="submit" name="psttcsv_export_submit" class="button-secondary" value="<?php _e( 'Export Now', 'post-to-csv' ); ?>" />
		...
		public function tab_comments() { ?>
            ...
            <table class="form-table psttcsv-table-settings" id="psttcsv_comments_form">
                ...
                <tr>
                    <th scope="row"><?php _e( 'Export to CSV', 'post-to-csv' ); ?></th>
                    <td>
                    <input type="submit" name="psttcsv_export_submit_comments" class="button-secondary" value="<?php _e( 'Export Now', 'post-to-csv' ); ?>" />
                        <div class="bws_info psttcsv_export_notice" style="display: none">
                                <strong><?php _e( 'Notice', 'post-to-csv' ); ?></strong>: <?php _e( "The plugin's settings have been changed.", 'post-to-csv' ); ?>
				                <a class="bws_save_anchor" href="#bws-submit-button"><?php _e( 'Save Changes', 'post-to-csv' ); ?></a>&nbsp;<?php _e( 'before export', 'post-to-csv' ); ?>.
                        </div>
            ...
            <div class="bws_pro_version_bloc">
                <div class="bws_pro_version_table_bloc">
                    ...
                    <tr valign="top">
                        ...
                        <td><fieldset>
                                <div class="psttcsv_div_select_all hide-if-no-js"><label><input disabled="disabled" id="psttcsv_select_all_status" class="psttcsv_select_all" type="checkbox" /> <strong><?php _e( 'All', 'post-to-csv' ); ?></strong></label></div>
                                <?php foreach ( $status as $value => $key ) { ?>
                                    <label>
                                        <input disabled="disabled" type="checkbox" name="psttcsv_status_woocommerce[]" value="<?php echo $value; ?>"  /> <?php echo $key; ?>
                                    </label>
                                    <br />
                                <?php } ?>
                            </fieldset>
                    ...
                    <tr valign="top">
                        ...
                        <td><fieldset>
                                <div class="psttcsv_div_select_all hide-if-no-js"><label><input disabled="disabled"  id="psttcsv_select_all_types" class="psttcsv_select_all" type="checkbox" /> <strong><?php _e( 'All', 'post-to-csv' ); ?></strong></label></div>
                                <?php foreach ( $woo_product_types as $types_id => $types_name ) { ?>
                                    <label>
                                        <input disabled="disabled"  type="checkbox" name="psttcsv_product_type_woocommerce[]" value="<?php echo $types_id; ?>" /> <?php echo $types_name; ?>
                                    </label><br />
                                <?php } ?>
                            </fieldset>
                        </td>
                    ...
                    <tr valign="top">
                        ...
                        <td><fieldset>
                                <div class="psttcsv_div_select_all hide-if-no-js"><label><input disabled="disabled"  id="psttcsv_select_all_category" class="psttcsv_select_all" type="checkbox" /> <strong><?php _e( 'All', 'post-to-csv' ); ?></strong></label></div>
                                    <label>
                                        <input disabled="disabled" type="checkbox" name="psttcsv_product_category_woocommerce[]" value="uncategorized" /> <?php echo _e( 'Uncategorized', 'post-to-csv' ); ?>
                                    </label><br />
                            </fieldset>
                        </td>
                    </tr>
                    <tr valign="top">
                        <th scope="row"><?php _e( 'Sort by', 'post-to-csv' ); ?></th>
                        <td><fieldset>
                                <label><input disabled="disabled" type="radio" name="psttcsv_order_woocommerce" value="ID"  /> <?php _e( 'ID', 'post-to-csv' ); ?></label><br />
                                <label><input disabled="disabled" type="radio" name="psttcsv_order_woocommerce" value="name" /> <?php _e( 'Name', 'post-to-csv' ); ?></label><br />
                                <label><input disabled="disabled" type="radio" name="psttcsv_order_woocommerce" value="type" /> <?php _e( 'Type', 'post-to-csv' ); ?></label><br />
                                <label><input disabled="disabled" type="radio" name="psttcsv_order_woocommerce" value="date" /> <?php _e( 'Date', 'post-to-csv' ); ?></label><br />
                                <label><input disabled="disabled" type="radio" name="psttcsv_order_woocommerce" value="modified" /> <?php _e( 'Modified', 'post-to-csv' ); ?></label><br />
                            </fieldset>
                        </td>
                    </tr>
                    <tr valign="top">
                        <th scope="row"><?php _e( 'Arrange by', 'post-to-csv' ); ?></th>
                        <td><fieldset>
                                <label><input disabled="disabled" type="radio" name="psttcsv_direction_woocommerce" value="ASC" /> <?php _e( 'ASC', 'post-to-csv' ); ?></label><br />
                                <label><input disabled="disabled" type="radio" name="psttcsv_direction_woocommerce" value="DESC" /> <?php _e( 'DESC', 'post-to-csv' ); ?></label><br />
                            </fieldset>
                        </td>
                    </tr>
                    <tr>
                        <th scope="row"><?php _e( 'Export to CSV', 'post-to-csv' ); ?></th>
                        <td>
                           	<input disabled="disabled" name="bws_restore_default" type="submit" class="button" value="<?php _e( 'Export Now', 'post-to-csv' ); ?>" />
                        </td>
                    </tr>
                    </table>
                </div>
